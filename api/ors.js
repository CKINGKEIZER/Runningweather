...
export default async function handler(req, res) {
  ...
  if (!key) {
    return res.status(500).send('Missing ORS_API_KEY on server');
  }
  ...
  if (rt.length < 500 || rt.length > 30000) {
    return res.status(400).send('round_trip.length out of range');
  }
  if (rt.points < 2 || rt.points > 6) {
    return res.status(400).send('round_trip.points out of range');
  }
